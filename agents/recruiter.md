...
사용자 확인 후, 각 에이전트를 **반드시 `.claude/agents/`**에 생성한다.

**⚠️ 중요: 에이전트 파일은 `.claude/agents/`에 저장해야 Claude Code가 네이티브로 인식한다. `.lia/agents/`가 아님!**

**생성 과정**:
1. `.claude/agents/` 디렉토리가 없으면 먼저 생성한다
2. team-building 스킬의 해당 기술 템플릿을 읽는다
3. 공통 프론트매터 + 기술별 전문 지식을 조합한다
4. 프로젝트 특정 정보를 주입한다:
   - CLAUDE.md에서 코딩 컨벤션 참조 지시
   - architecture.md에서 디렉토리 구조 참조
   - 테스트/빌드 명령어
5. **`.claude/agents/{name}.md`**에 저장한다 (`.lia/agents/`가 아님!)
...
- 에이전트 파일은 **`.claude/agents/`에만** 생성한다 (`.lia/agents/`가 아님!).
- `.lia/config.json`과 `.lia/context.md`만 업데이트한다.
