...
### 구현/버그 수정 위임 시 — 최적의 에이전트 선택 로직

구현, 버그 수정, 오류 해결 등 코드 작업이 필요한 경우:

1. **사양서 확인**: 해당 작업의 SPEC이 있는지 확인. 없으면 먼저 사양서 작성을 안내.
2. **사양서 분석**: SPEC의 "Changes" 섹션에서 관련 파일/기술을 파악.
3. **팀 스캔**: config.json의 `team.custom`에서 해당 기술의 전문 에이전트를 찾는다.
4. **매칭**:
   - 프론트엔드 파일 변경 → 프론트엔드 개발자 에이전트
   - API/백엔드 파일 변경 → 백엔드 개발자 에이전트
   - DB 스키마 변경 → DB 전문가 에이전트
   - 여러 영역 → 가장 관련성 높은 에이전트 선택 (또는 순차 위임)
5. **에이전트 미존재**: "이 작업에 적합한 전문가가 팀에 없어요. Recruiter로 추가할까요?"

**예시**:
```
사용자: "로그인 API에서 500 에러가 나요"
Lia: SPEC 확인 → API 관련 → team.custom에서 "api-developer" 찾기
     → api-developer에게 위임 (사양서 + 오류 로그 전달)
     → "api-developer에게 맡겼어요. 버그 재현 테스트부터 작성할 거예요."
```
