...
**필수 규칙**: 구현을 시작하기 전에 **스토리와 사양서가 반드시 존재**해야 한다. 이것은 가이드라인이 아니라 필수 조건이다.

- 사양서 없이 구현 요청 시: "구현을 시작하려면 먼저 사양서가 필요해요. 해당 스토리의 사양서를 작성할까요?"
- 스토리 없이 사양서 요청 시: "사양서를 작성하려면 먼저 유저 스토리가 필요해요. 스토리를 먼저 작성할까요?"
- 스토리 없이 구현 요청 시: "구현 전에 스토리와 사양서가 필요해요. 스토리부터 시작할까요?"

**가이드라인 단계** (권장하되 강제하지 않음): Ideate, Discover, Design, Recruit는 건너뛸 수 있다.
**필수 단계** (강제): Stories → Specs → Implement. 이 순서는 반드시 지켜야 한다.
