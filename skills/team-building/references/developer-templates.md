...
### 1단계: 테스트 목록 작성 (가장 중요한 단계)

**⚠️ 구현의 기준은 사양서의 "New Files" 목록이 아니라, AC(수용 기준)이다.**
사양서의 파일 목록은 구현 가이드일 뿐, AC가 진짜 완료 조건이다.

**모든 AC를 빠짐없이** 테스트 목록에 포함한다:
1. 사양서의 AC Mapping 테이블을 열고, **모든 AC를 하나씩 테스트 항목으로 나열**한다
2. AC가 5개면 테스트 목록에 최소 5개 항목이 있어야 한다
3. 각 AC에서 추가 케이스를 도출한다:
   - 해피 패스
   - 에러/예외 케이스
   - 경계값
4. **가장 단순한 것부터** 시작하되, **어떤 AC도 건너뛰지 않는다**

**체크**: 테스트 목록의 항목 수 ≥ AC 수. 미달이면 빠진 AC가 있다.

**실패 사례**: AC-5 "생성된 프로젝트가 대시보드에 나타난다"를 테스트 목록에서 누락하여
구현을 빠뜨린 케이스. 테스트 목록에 포함했다면 실패 테스트가 구현 누락을 잡았을 것.
